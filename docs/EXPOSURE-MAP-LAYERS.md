...
1. 每个国家只显示一个点。
2. 先在国家内按城市聚合。
3. 选择 `Instances` 最高的城市作为锚点城市。
4. 如果 `Instances` 相同，再比较 `Seen`。
5. 如果仍然相同，再按城市名排序。
6. 在选中的城市内，再选一条真实代表点。
7. 代表点的选择顺序是：`Instances` > `Seen` > `IP`。
8. 最终直接使用这条真实点的原始经纬度作为国家锚点。

设计意图：

- 世界层表达的是“这个国家哪里最值得看”，不是国家几何中心，也不是首都。
- 锚点必须落在真实观测点上，避免点落到海里或落出国界。
- 点大小不表示数量，只服务于世界视角下的大概分布阅读。
...
- 后续如果继续调图层，优先调 `Country / Region`、`City`、`Single Instance`。
