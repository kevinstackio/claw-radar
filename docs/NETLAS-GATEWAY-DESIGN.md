...
- 业务侧无感知：1 把 key 和多把 key 用同一套接口
- 每天定时同步 1 次，但同步量动态调整
- 不超额、不打爆、不断更
- 对 `402/429/504/超时/未知` 有统一语义
- 同步前先额度校准，保留安全冗余
...
- `Netlas Gateway`：统一请求、重试、错误映射、选 key
- `Quota Ledger`：日/月额度账本（key 级 + 全局）
- `Quota Calibrator`：同步前额度校准与漂移检测
...
`score = 剩余额度率 + 欠用量 - 错误惩罚 - 冷却惩罚`

规则：

- 冷却/超额/封禁 key 不参与分配
- `429` 按 `Retry-After` 进入冷却
